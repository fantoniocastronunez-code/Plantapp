...
export const identifyPlantFromImage = async (base64Data: string, mimeType: string): Promise<GeminiPlantInfo> => {
  const prompt = `Eres un experto botánico. Identifica la planta de esta imagen. Proporciona su nombre común (como apodo o nombre coloquial), su especie científica, la frecuencia de riego en días y la frecuencia de fertilización en días. Devuelve la información en formato JSON siguiendo estrictamente este esquema.`;
    ...
    config: {
      ...
      responseSchema: {
        ...
        properties: {
          common_name: {
            type: Type.STRING,
            description: "Nombre común o coloquial de la planta",
          },
          species: {
            type: Type.STRING,
            description: "Nombre científico de la planta",
        ...
        required: ["common_name", "species", "watering_frequency", "fertilizer_frequency", "description"],
      },
    },
  });

  if (!response.text) {
    throw new Error('No se pudo generar una respuesta de Gemini a partir de la imagen.');
  }
